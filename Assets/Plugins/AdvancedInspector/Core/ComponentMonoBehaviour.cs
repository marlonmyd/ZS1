...
namespace AdvancedInspector
{
    ...
    public abstract class ComponentMonoBehaviour : MonoBehaviour
    {
        ...
        private static List<FieldInfo> GetFields(Type type, bool recursive)
        {
            ...
            if (recursive)
                infos = type.GetFields(BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Instance).ToList();
            else
                infos = type.GetFields(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.FlattenHierarchy).ToList();
